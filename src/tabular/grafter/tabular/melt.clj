...
(defn mapcat-rows
  ...
  output by the given transform function.  dataset: The dataset to
  transform.  columns: The collection of columns in the output
  dataset.  f: function (Row -> Seqable Row) which transforms each row
  in the input into a sequence of new rows in the output. Each output
  row should contain the columns passed in the columns parameter."
  [dataset columns f]
